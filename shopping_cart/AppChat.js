...
const AppChat = () => {
    ...
    const [getColorText, setColorText] = useState([]);
    let colorDiv = "";
    let allText = {};
    ...
    function addText() {
        allText = {
            messageText: inputs.messageText.current.value
        }
        if(allText.messageText === ""){
            alert("Your message is empty")
        } else {
        setColorText([...getColorText,{col: colorDiv, text: allText.messageText}])
        console.log(getColorText)
        inputs.messageText.current.value = "";
        }
    }

    function getData(chosenColor) {
        console.log(chosenColor, "spalva")
        colorDiv = chosenColor;
    }

    function divBox() {
        if (!!getColorText) {
            return getColorText.map((x, index) =>  <div key={index} style={{border: "1px solid black", backgroundColor: x.col, height: "auto", width: "fit-content", margin: "5px", padding: "5px", borderRadius: '10px', fontWeight: 'bold', color: x.col==="#000000" ? "white" : "black", alignSelf: index % 2 === 0 ? 'self-end' : ''}} > {x.text} </div>)
        }
    }
    ...
    return (
        <div className="d-flex">
            ...
            <Colors colorArr={colorArray} sendData={getData}/>
            <div className="d-flex column justify-center align-center">
                <div className="box2 d-flex column" >
                    {divBox()}
                </div>
                <textarea placeholder="Write your message here" ref={inputs.messageText}/>
